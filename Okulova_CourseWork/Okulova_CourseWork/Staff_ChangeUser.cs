...
{
    public partial class Staff_ChangeUser : Form
    {
        private DB db = new DB();
        public Staff_ChangeUser()
        {
            InitializeComponent();
        ...
        private void Foundbutton_Click(object sender, EventArgs e)
        {
            if (IDtextbox.Text != "")
            {
                ...
                else
                {
                    ...
                    {
                        panel2.Show();
                        BirthdayPicker.Value = (from c in db.GetUsersTable()
                                                where c.Id == Convert.ToInt32(IDtextbox.Text)
                                                select c.Birthday).FirstOrDefault();
                        Searchtextbox.Enabled = false;
                        IDtextbox.Enabled = false;
                        Foundbutton.Enabled = false;
                        dataGrid.DataSource = from c in db.GetUsersTable()
                                              where c.Id == Convert.ToInt32(IDtextbox.Text)
                                              select c;
                    }
                }
        ...
        private void OKbutton_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Принять изменения?", "Вы уверены?",
                MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                db.ChangeUser(Convert.ToInt32(IDtextbox.Text), FirstNametextbox.Text,
                      LastNametextbox.Text, Phonetextbox.Text, Adresstextbox.Text, BirthdayPicker.Value);
